// Dire Spider for 2025
// Now with poisonous web spray!
...
export function render2D(index,x,y) {
  ...
  rf = clamp(hypot(x,y),0.0, 1.0)
  r = rf - x / 6; 
  r2 = hypot(x+0.12,y);
  a = (atan2(y, x) + PI) / PI2
  
  // ridge noise from Wizard's "Eye of Sauron" for swirling poision mist
  f = perlinRidge(a * aDensity,rf * rDensity - rTime , morphTime, 2, .5, 1.1, 3)
  // fade the poison spray
  f = f * f * f * f
  rf = 1 - rf
  f *= rf
